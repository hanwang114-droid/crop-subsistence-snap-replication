...
\begin{{tablenotes}}
\footnotesize
\item Notes: The table summarizes the baseline raw-productivity Stone--Geary calibration. Productivity is not normalized to one in 1960. The common parameter $\alpha$ is chosen to minimize full-panel squared prediction error. Soft-DTW matching scores are computed after calibration and are not used to choose $\alpha$ or $\bar a_s$.
\end{{tablenotes}}
\end{{threeparttable}}
\end{{table}}
"""
    ...
    lb = '\\\\'
    lines = [r"\begin{table}[!htbp]", r"\centering", r"\caption{Main Calibration Results and Early-Year Robustness}", r"\label{tab:main_calibration_results}", r"\begin{threeparttable}", r"\small", r"\begin{tabular}{lccc}", r"\toprule", f"Object & 1960 only & Baseline: 1960, 1970 & 1960, 1970, 1980 {lb}", r"\midrule"]
    ...
    lines += [r"\bottomrule", r"\end{tabular}", "", r"\begin{tablenotes}", r"\footnotesize",
              r"\item Notes: The calibration uses raw USDA--ERS state labor productivity, defined as agricultural output quantity divided by labor input quantity, rather than a within-state 1960-normalized productivity series. The baseline calibration recovers $\bar a_s(\alpha)$ using the 1960 and 1970 calibration moments. The adjacent columns report robustness checks using only 1960 and using 1960, 1970, and 1980. In all columns, the common parameter $\hat\alpha$ is chosen to minimize full-panel squared prediction error. Conditional on each candidate $\alpha$, $\bar a_s(\alpha)$ is recovered analytically using the indicated early calibration moments and the corresponding raw productivity values. Soft-DTW is computed only after calibration.",
