...
MAIN_SELECTED_MATCH_THRESHOLD = 95.0
...
def write_panel_latex(ptable, caption, label, tex_path, sample_note):
    ...
    lines.append(
        r"\emph{Notes:} Robust HC1 standard errors are reported in parentheses. "
...
def latex_num(coef, se, p):
    st = stars(p)
    coef_cell = f"${coef:.3f}^{{{st}}}$" if st else f"${coef:.3f}$"
    return coef_cell, f"$({se:.3f})$"


def write_main_results_summary_table(interaction_output):
    """Write a compact six-column summary table from generated outputs."""
    iv_path = ROOT / "derived" / "regression_work" / "selected_gt95_log_snap_rate_log_abar_log_crop_all_results_long_with_first_stage.csv"
    if not iv_path.exists():
        raise FileNotFoundError(f"Missing generated main regression results: {iv_path}")

    iv = pd.read_csv(iv_path)
    interaction = interaction_output["results_table"].copy()

    def pick_iv(model, weighting):
        row = iv[
            (iv["model"] == model) &
            (iv["fe_spec"] == "Region FE") &
            (iv["weighting"] == weighting)
        ]
        if row.empty:
            raise ValueError(f"Missing summary row: {model}, {weighting}")
        return row.iloc[0]

    def pick_interaction(variable):
        row = interaction[
            (interaction["panel"] == "Soybean Share Interaction") &
            (interaction["fe_spec"] == "Region FE") &
            (interaction["estimator"] == "WLS") &
            (interaction["variable"] == variable)
        ]
        if row.empty:
            raise ValueError(f"Missing soybean interaction row: {variable}")
        return row.iloc[0]

    ols = pick_iv("Baseline", "Unweighted")
    fs = pick_iv("Log Soybean Acreage Intensity IV", "Unweighted")
    fs_wls = pick_iv("Log Soybean Acreage Intensity IV", "WLS")
    iv_unw = pick_iv("Log Soybean Acreage Intensity IV", "Unweighted")
    iv_wls = pick_iv("Log Soybean Acreage Intensity IV", "WLS")
    int_x = pick_interaction(XVAR)
    int_share = pick_interaction("share_soybean")
    int_term = pick_interaction("log_abar_x_share_soybean")

    ols_c, ols_s = latex_num(ols["coef"], ols["se"], ols["p"])
    fs_c, fs_s = latex_num(fs["first_stage_coef"], fs["first_stage_se"], fs["first_stage_p"])
    fsw_c, fsw_s = latex_num(fs_wls["first_stage_coef"], fs_wls["first_stage_se"], fs_wls["first_stage_p"])
    iv_c, iv_s = latex_num(iv_unw["coef"], iv_unw["se"], iv_unw["p"])
    ivw_c, ivw_s = latex_num(iv_wls["coef"], iv_wls["se"], iv_wls["p"])
    intx_c, intx_s = latex_num(int_x["coef"], int_x["se"], int_x["p"])
    share_c, share_s = latex_num(int_share["coef"], int_share["se"], int_share["p"])
    term_c, term_s = latex_num(int_term["coef"], int_term["se"], int_term["p"])

    lines = [
        r"\begin{table}[!htbp]",
        r"\centering",
        r"\caption{Main Results: Soybean Commercialization, Subsistence Intensity, and SNAP Participation}",
        ...
        r"\hline\hline",
        r" & (1) & (2) & (3) & (4) & (5) & (6) \\",
        r" & OLS & First Stage & First Stage WLS & IV & IV-WLS & Interaction WLS \\",
        r"\hline",
        r"Dependent variable & $\log(\text{SNAP rate})$ & $\log(\hat{\bar a}_s)$ & $\log(\hat{\bar a}_s)$ & $\log(\text{SNAP rate})$ & $\log(\text{SNAP rate})$ & $\log(\text{SNAP rate})$ \\",
        rf"$\log(\hat{{\bar a}}_s)$ & {ols_c} & & & {iv_c} & {ivw_c} & {intx_c} \\",
        rf"& {ols_s} & & & {iv_s} & {ivw_s} & {intx_s} \\",
        rf"$\log(1+\text{{soybean acres per 1,000 state land acres}})$ & & {fs_c} & {fsw_c} & & & \\",
        rf"& & {fs_s} & {fsw_s} & & & \\",
        rf"Soybean share & & & & & & {share_c} \\",
        rf"& & & & & & {share_s} \\",
        rf"$\log(\hat{{\bar a}}_s)\times\text{{Soybean share}}$ & & & & & & {term_c} \\",
        rf"& & & & & & {term_s} \\",
        rf"First-stage F-statistic & & {fs['first_stage_F']:.2f} & {fs_wls['first_stage_F']:.2f} & {iv_unw['first_stage_F']:.2f} & {iv_wls['first_stage_F']:.2f} & \\",
        r"Region fixed effects & Yes & Yes & Yes & Yes & Yes & Yes \\",
        r"Reliability weights & No & No & $\text{Match}/100$ & No & $\text{Match}/100$ & $\text{Match}/100$ \\",
        rf"Observations & {int(ols['nobs'])} & {int(fs['nobs'])} & {int(fs_wls['nobs'])} & {int(iv_unw['nobs'])} & {int(iv_wls['nobs'])} & {int(int_x['nobs'])} \\",
        r"\hline\hline",
        r"\end{tabular}",
        r"\end{adjustbox}",
        r"\vspace{0.25em}",
        ...
        r"\footnotesize",
        r"\emph{Notes:} The dependent variable in the SNAP specifications is the log SNAP participation rate, defined as SNAP persons divided by the July 1, 2025 Census resident population estimate. The first-stage dependent variable is the log calibrated subsistence index, $\log(\hat{\bar a}_s)$. The excluded instrument is log one plus historical soybean acres per 1,000 state land acres, averaged over 1926--1940. Soybean share is the share of soybean acreage in total cotton, corn, and soybean acreage over 1926--1940. The interaction specification includes $\log(\hat{\bar a}_s)$, soybean share, and their interaction. The sample includes states with post-calibration Soft-DTW matching scores greater than 95 percent. Reliability-weighted specifications use the post-calibration Soft-DTW matching score as weights. Robust standard errors are reported in parentheses. $^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$.",
        r"\end{minipage}",
        r"\end{table}",
        "",
    ]

    outpath = MANUSCRIPT_REGRESSION_DIR / "main_results_summary_table.tex"
...
selected_gt95_dir = os.path.join(output_dir, "gt95_main")

selected_gt95_output = estimate_interactions_for_sample(
    base=base_reg,
    sample_key="gt95_int",
    sample_label="Selected-State, Match Score $>$ 95\\%",
    sample_note="The selected sample includes states with post-calibration Soft-DTW matching scores greater than 95 percent. ",
    sample_dir=selected_gt95_dir,
    threshold=MAIN_SELECTED_MATCH_THRESHOLD,
)

# Copy only the manuscript-facing interaction table.
interaction_source = selected_gt95_output["tex_path"]
...
summary_target = write_main_results_summary_table(selected_gt95_output)

print("\nFinal manuscript-facing interaction output:")
print(interaction_target)
print("Final manuscript-facing summary output:")
