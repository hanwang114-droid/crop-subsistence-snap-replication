...
GAZETTEER_DIR = CLEAN / "census_gazetteer"

for d in [MOTIVATION_DIR, CROP_DIR, SNAP_DIR, STRUCT_DIR, GAZETTEER_DIR]:
...
def clean_county_gazetteer() -> None:
    """Clean Census county Gazetteer land area into county and state denominators."""
    src = RAW / "census_gazetteer" / "census_2024_county_gazetteer.zip"
    require(src, "Census 2024 county Gazetteer zip")

    with zipfile.ZipFile(src) as zf:
        txt_files = [name for name in zf.namelist() if name.lower().endswith(".txt")]
        if not txt_files:
            raise FileNotFoundError(f"No text file found inside {src}")
        gaz = pd.read_csv(zf.open(txt_files[0]), sep="\t", dtype=str)

    gaz.columns = [col.strip() for col in gaz.columns]
    geoid_col = "GEOID" if "GEOID" in gaz.columns else "GEOIDFP"
    if "ALAND" in gaz.columns:
        land_acres = pd.to_numeric(gaz["ALAND"], errors="coerce") / SQMETERS_PER_ACRE
    elif "ALAND_SQMI" in gaz.columns:
        land_acres = pd.to_numeric(gaz["ALAND_SQMI"], errors="coerce") * 640
    else:
        raise ValueError("County Gazetteer must include ALAND or ALAND_SQMI.")

    county = pd.DataFrame({
        "county_fips_full": gaz[geoid_col].astype(str).str.zfill(5),
        "state_fips": gaz[geoid_col].astype(str).str.zfill(5).str[:2],
        "land_acres": land_acres,
    })
    county["state"] = pd.to_numeric(county["state_fips"], errors="coerce").map(FIPS_TO_ABBR)
    county["log_land_acres"] = np.where(county["land_acres"] > 0, np.log(county["land_acres"]), np.nan)
    county.to_csv(GAZETTEER_DIR / "county_land_area_2024.csv", index=False)

    state = (
        county.dropna(subset=["state"])
        .groupby("state", as_index=False)["land_acres"]
        .sum()
    )
    state["log_land_acres"] = np.where(state["land_acres"] > 0, np.log(state["land_acres"]), np.nan)
    state.to_csv(GAZETTEER_DIR / "state_land_area_2024.csv", index=False)
    print("Wrote clean Census Gazetteer land-area files.")
