...
Inputs:
  ...
  * SNAP participation rate = January 2026 SNAP persons / July 1, 2025 population;
  * historical crop acreage averaged over 1926--1940;
...
The crop instruments use log(1 + acreage), which keeps zero-acreage states in
the sample and reduces leverage from very large acreage values.
"""
# -----------------------------------------------------------------------------
# READER GUIDE
# -----------------------------------------------------------------------------
...
# historical log(1 + acreage) instruments, and reliability weights. It writes
# only publication-facing regression tables to outputs/regression/.
# -----------------------------------------------------------------------------

# ============================================================
# MATCHING-SCORE-SELECTED AND FULL-SAMPLE IV REGRESSIONS
# LOG SUBSISTENCE PARAMETER + LOG HISTORICAL CROP ACREAGE IVs
# ============================================================
#
...
#   Instruments:  log historical crop acreage, averaged over 1926--1940
...
# more direct measure of historical crop scale.
# ============================================================
...
# Use log1p for crop acreage so that zero-acre states remain in the sample.
# log1p(x) = log(1 + x). If you want plain log acreage instead, set False.
...
IV_SPECS = [
    ("Log Cotton Acres IV", "log_cotton_acres", "Log cotton acres"),
    ("Log Corn Acres IV", "log_corn_acres", "Log corn acres"),
    ("Log Soybean Acres IV", "log_soybean_acres", "Log soybean acres"),
...
def instrument_display_name(instrument):
    labels = {
        "None": "None",
        "log_cotton_acres": "Log cotton acres",
        "log_corn_acres": "Log corn acres",
        "log_soybean_acres": "Log soybean acres",
        SUIT: "Suitability",
    }
...
def write_latex_table_for_output(table, caption, label, filename, output_dir_for_run, sample_note):
    ...
    latex_full = (
        ...
        f"{sample_note} "
        "Crop instruments are log(1 + historical crop acreage) averaged over 1926--1940. "
...
for crop in CROPS:
    ...
    if USE_LOG1P_CROP_ACRES:
        crop_wide[f"log_{crop}_acres"] = np.log1p(crop_wide[crop])
    else:
        crop_wide[f"log_{crop}_acres"] = np.where(crop_wide[crop] > 0, np.log(crop_wide[crop]), np.nan)
...
# Census resident population estimates. For January 2026 SNAP, the closest
...
MODEL_PANEL_ORDER = [
    ...
    ("Log Cotton Acres IV", "Panel B. Log Cotton Acres IV"),
    ("Log Corn Acres IV", "Panel C. Log Corn Acres IV"),
    ("Log Soybean Acres IV", "Panel D. Log Soybean Acres IV"),
    ("Suitability IV", "Panel E. Suitability IV"),
]

FIRST_STAGE_PANEL_ORDER = [
    ("Log Cotton Acres IV", "Panel A. Log Cotton Acres First Stage"),
    ("Log Corn Acres IV", "Panel B. Log Corn Acres First Stage"),
    ("Log Soybean Acres IV", "Panel C. Log Soybean Acres First Stage"),
...
def run_sample_analysis(base_reg, sample_key, sample_title, sample_note, output_dir_for_run):
    ...
    table_specs = [
        ...
        ("Log Cotton Acres IV", f"{sample_title} IV Estimates: Log Cotton Acres: SNAP Participation Rate", f"tab:{sample_key}_log_cotton_acres_iv", f"{sample_key}_log_cotton_acres_iv_table"),
        ("Log Corn Acres IV", f"{sample_title} IV Estimates: Log Corn Acres: SNAP Participation Rate", f"tab:{sample_key}_log_corn_acres_iv", f"{sample_key}_log_corn_acres_iv_table"),
        ("Log Soybean Acres IV", f"{sample_title} IV Estimates: Log Soybean Acres: SNAP Participation Rate", f"tab:{sample_key}_log_soybean_acres_iv", f"{sample_key}_log_soybean_acres_iv_table"),
...
# ============================================================
# RUN SELECTED SAMPLE: MATCHING SCORE > 90
# ============================================================

selected_gt90_states = sorted(base_reg.loc[base_reg[MATCH] > SELECTED_MATCH_THRESHOLD, "state"].dropna().unique())
if len(selected_gt90_states) == 0:
    raise ValueError(f"No states satisfy matching_score_percent > {SELECTED_MATCH_THRESHOLD}.")

selected_gt90_reg = base_reg[base_reg["state"].isin(selected_gt90_states)].copy()
selected_output_dir = os.path.join(output_dir, "selected_gt90_log_snap_rate_log_abar_log_crop")

selected_results_table, selected_all_tables = run_sample_analysis(
    base_reg=selected_gt90_reg,
    sample_key="selected_gt90_log_snap_rate_log_abar_log_crop",
    sample_title="Selected-State, Match Score $>$ 90\\%",
...
selected_results_table.to_csv(os.path.join(output_dir, "selected_gt90_log_snap_rate_log_abar_log_crop_all_results_long_with_first_stage.csv"), index=False)
selected_results_table[selected_results_table["instrument"] != "None"].to_csv(os.path.join(output_dir, "selected_gt90_log_snap_rate_log_abar_log_crop_first_stage_only_summary.csv"), index=False)
pd.DataFrame({"state": selected_gt90_states}).to_csv(os.path.join(output_dir, "selected_gt90_log_snap_rate_log_abar_log_crop_states_used.csv"), index=False)
...
def write_compact_soybean_threshold_table(base_reg, output_dir_for_run):
    ...
    lines.append(r"\footnotesize")
    lines.append(
        r"\item Notes: Robust standard errors are reported in parentheses. "
        r"$^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$. "
        r"The dependent variable is log SNAP participation rate, defined as the logarithm of January 2026 SNAP participants divided by the July 1, 2025 Census resident population estimate. "
        r"The endogenous regressor is $\log(\hat{\bar a}_s)$. "
        r"The excluded instrument is the logarithm of one plus average historical soybean acreage over 1926--1940. "
...
main_source = os.path.join(
    selected_output_dir,
    "selected_gt90_log_snap_rate_log_abar_log_crop_baseline_iv_panel_table.tex"
)
...
first_stage_source = os.path.join(
    selected_output_dir,
    "selected_gt90_log_snap_rate_log_abar_log_crop_first_stage_panel_table.tex"
)
