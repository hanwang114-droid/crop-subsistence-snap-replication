...
def make_county_snap_map():
    """Create a county-level SNAP household receipt-rate map in the same style."""
    county_panel_path = CLEAN / "county_historical_soybean_panel.csv"
    if not county_panel_path.exists():
        print(f"Skipping county SNAP map: missing {county_panel_path}. Run 07_county_level.py first.")
        return
    ...
    counties = gpd.read_file(county_shape_path)
    counties = counties[~counties["STUSPS"].isin(["AK", "HI", "PR", "GU", "VI", "MP", "AS"])].copy()
    counties["county_fips_full"] = counties["GEOID"].astype(str).str.zfill(5)

    panel = pd.read_csv(county_panel_path, dtype={"county_fips_full": str})
    panel["county_fips_full"] = panel["county_fips_full"].astype(str).str.zfill(5)
    if "snap_rate" not in panel.columns:
        print("Skipping county SNAP map: snap_rate not found in county panel.")
        return

    map_data = counties.merge(
        panel[["county_fips_full", "snap_rate"]],
        on="county_fips_full",
        how="left"
    ).to_crs("EPSG:5070")

    # Start above pure white so missing counties remain visually distinct.
    cmap = truncate_colormap(plt.get_cmap("Greys"), 0.18, 0.85)
    fig, ax = plt.subplots(figsize=(13, 8))
    map_data.plot(
        column="snap_rate",
        ax=ax,
    ...
    ax.set_title(
        "County SNAP Household Receipt Rate",
        fontsize=17,
        fontweight="bold",
        pad=18
    )
    ax.axis("off")
    n_counties = map_data["snap_rate"].notna().sum()
    note = (
        "Notes: Mainland U.S. counties only; Alaska and Hawaii omitted for readability. "
        f"Counties grouped into five quantile bins. Missing counties are white with diagonal hatching. N = {n_counties}."
    )
    ax.text(0.01, -0.03, note, transform=ax.transAxes, fontsize=9, ha="left", va="top")
    plt.tight_layout()
    outpath = output_dir / "informative_map_county_snap_rate.png"
